...
| `ir` (response shapes) | `spargen/src/ir/media.rs` in-module + `spargen/tests/e2e.rs` | `Responses::success`/`error`: the body count that picks unit, single, or enum; precedence order (exact, then range, then `default` last on the error side); `default` as the success body only when `by_status` is empty; `default` beside explicit statuses never entering the success enum. In `e2e.rs`, the generated dispatch for those shapes driven over a mock (`getMulti`, `getMultiDefault`), including an undocumented 2xx. |
