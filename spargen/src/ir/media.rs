...
pub(crate) struct RequestBody {
    ...
    /// The body's type, or `None` for a body declared without a schema. The type of an
    /// octet-stream body always has a `bytes::Bytes` definition, but the reference may still be
    /// nullable, which the emitter does not handle yet (#104).
