...
impl TypeGraph {
    ...
    /// `oneOf` variant became the union itself. A dedicated variant makes every one of those a
    /// compile error until the site says what it does with a back edge, so the audit is the
